...
def read_xml_annotation(Path, fileName):
    ...
    for object in Path.findall('object'):
        ...
        x_min = bandbox.find('xmin').text
        x_max = bandbox.find('xmax').text
        y_min = bandbox.find('ymin').text
        y_max = bandbox.find('ymax').text
        bandboxes_list.append([name, x_min, y_min, x_max, y_max])
    try:
        Path.find('object').find('bndbox')
    except:
        print(fileName + "'s bandbox is empty")
        pass
    return bandboxes_list
...
def crate_xml_file(write_path, file_name_, new_bndbox, Folder=None, img_path=None, img_size=None):
    try:
        os.mkdir(write_path)
    except:
        pass
    ...
    if new_bndbox == emptyList:
        ...
        elif img_path is None and img_size is not None:
            img_height = img_size[0]
            img_weight = img_size[1]
            size = ET.SubElement(root, 'size')
            width = ET.SubElement(size, 'width')
            width.text = str(img_weight)
            height = ET.SubElement(size, 'height')
    ...
    for filename in filename_for_label:
        ...
        for epoch in range(AUGLOOP):
            ...
            for i in range(len(bndbox)):
                ...
                new_bandbox_list.append([n_x1, n_y1, n_x2, n_y2])
            print(new_bandbox_list)
            # exit()
            ...
            change_bndbox_to_newbndbox(source_xml_path, AUG_XML_PATH, new_bandbox_list, filename, new_filename + ".xml")
...
def xywh2xyxy(xywh, imgsz):
    ...
    imgweight = imgsz[1]
    imgheight = imgsz[0]
    xmin = str('%.6f' % (max(((center_X * imgweight) - ((weights * imgweight) / 2)), 0)))
    xmax = str('%.6f' % (min(((center_X * imgweight) + ((weights * imgweight) / 2)), imgweight)))
    ymin = str('%.6f' % (max(((center_y * imgheight) - ((height * imgheight) / 2)), 0)))
    ymax = str('%.6f' % (min(((center_y * imgheight) + ((height * imgheight) / 2)), imgheight)))
    # print(xmin,ymin,xmax,ymax)
    return xmin, ymin, xmax, ymax


def yolo2voc(yolo_labeldir, output_dir, file_name, img_dir, class_list):
    with open(yolo_labeldir + file_name, "r") as txtf:
        img_end = "." + read_fileName_in_path(img_dir)[0].split(".")[1]
        bndboxes = []
        for line in txtf.readlines():
            if line != "":
                line_lst = line.split(" ")
                clss_name = class_list[int(line_lst[0])]
                ...
                imgname = file_name.split(".")[0] + img_end
                print(imgname)
                # img = cv2.imread(img_dir + imgname)
                # imgheight,imgweight = img.shape[0],img.shape[1]
                img = Image.open(img_dir + imgname)
                imgweight, imgheight = img.size
                imgsz = [imgheight, imgweight]
                xmin, ymin, xmax, ymax = xywh2xyxy(xywh, imgsz)
                bndbox = [clss_name, xmin, ymin, xmax, ymax]
                bndboxes.append(bndbox)
        print(bndboxes)
        crate_xml_file(output_dir, file_name.split(".")[0] + ".xml", bndboxes, img_size=imgsz)
        # break

# creat_nemo_label_json
